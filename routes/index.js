...
mongodb.MongoClient.connect(url, {useNewUrlParser: true}, function(err, client){
    if (err){
        ...
        console.log("Connected to database5");
        db = client.db("database5");
    }
});
...
router.post('/removetask', function (req, res) {
    ...
    db.collection('database5').deleteOne({_id : id});
...
router.get('/listTasks', function(req, res){
    db.collection("database5").find({}).toArray(function (err, d) {
...
router.post("/updatetask", function(req, res){
    ...
    db.collection("database5").updateOne(filter, update);
    res.redirect('/listTasks');
});


router.post('/addtask', function(req, res){
    db.collection("database5").insertOne({
        name : req.body.name,
        due : req.body.due,
        desc : req.body.desc,
